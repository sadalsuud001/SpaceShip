...
def process_sprite_group(my_set,canvas):
    ...
    for q in list(my_set):
        q.draw(canvas)
        if q.update():
            my_set.remove(q)

        
def group_collide(my_set,other_object):
    flag = False
    for q in list(my_set):
        if q.collide(other_object):
            flag = True
            my_set.remove(q)
...
def group_group_collide(group1,group2):
    ...
    for q in list(group1):
        
        if group_collide(group2,q):
            num = num + 1
            group1.discard(q)
...
class Sprite:
    ...
    def collide(self,other_object):
        ...
        if dist(a,self.pos)<b+self.radius:
